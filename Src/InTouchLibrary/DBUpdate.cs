// Decompiled with JetBrains decompiler
// Type: InTouchLibrary.DBUpdate
// Assembly: InTouchLibrary, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 2EC5C0AD-EDBE-443B-8339-00330D273297
// Assembly location: C:\Users\Admin\Desktop\RE\IntouchApp\InTouchLibrary.dll

#nullable disable
namespace InTouchLibrary
{
  public class DBUpdate
  {
    public string modifiedContactID;
    public string modifiedContactMCI_CID;
    public int modifiedContactDirty;
    public string modifiedContactContactHash;
    public string modifiedContactImageHash;
    public Contacts contact;
  }
}
