...
namespace SQLite
{
  public class SQLiteConnection : IDisposable
  {
      ...
      bool storeDateTimeAsTicks = false)
    {
      ...
      SQLite3.SetDirectory(2U, ApplicationData.Current.TemporaryFolder.Path);
      IntPtr db;
    ...
    public IEnumerable<TableMapping> TableMappings
    {
      get
      {
        return this._tables == null ? Enumerable.Empty<TableMapping>() : (IEnumerable<TableMapping>) this._tables.Values;
      }
    }
    ...
    {
      if (!((property.Body.NodeType != ExpressionType.Convert ? property.Body as MemberExpression : ((UnaryExpression) property.Body).Operand as MemberExpression).Member is PropertyInfo member))
    ...
    public int Insert(object obj, string extra, Type objType)
    {
      ...
      {
        for (; objType != null; objType = typeInfo.BaseType)
        {
    ...
    public int Update(object obj, Type objType)
    {
      ...
      try
      {
        return this.Execute(query, objectList.ToArray());
      }
      catch (SQLiteException ex)
      {
